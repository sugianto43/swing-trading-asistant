...
## Objective
Build the Alerts screen (list, filters) and wire live updates via Phase 10's existing SSE endpoint (`GET /alerts/stream`), rendering deduplicated alerts as the backend produces them without re-fabricating or re-deduplicating client-side.
